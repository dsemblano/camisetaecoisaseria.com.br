...
final class WOOBE_HISTORY extends WOOBE_EXT {
    ...
    public function __construct() {
        ...
        //tabs
        $this->add_tab($this->slug, 'panel', __('History', 'woocommerce-bulk-editor'));
    ...
    public function woobe_ext_scripts() {
        ...
        <script>
            ...
            lang.<?php echo $this->slug ?>.reverting = '<?php _e('Reverting', 'woocommerce-bulk-editor') ?> ...';
            lang.<?php echo $this->slug ?>.reverted = '<?php _e('Reverted!', 'woocommerce-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.wait_until_finish = '<?php _e('Wait please while data reverting is going!', 'woocommerce-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.clearing = '<?php _e('History clearing ...', 'woocommerce-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.cleared = '<?php _e('History is cleared!', 'woocommerce-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.history_is_going = "<?php echo __('ATTENTION: History operation is going!', 'woocommerce-bulk-editor') ?>";
        </script>
        <?php
    }
  ...
  KEY `user_id` (`user_id`)
        ...
        if ($wpdb->query($sql1) === false) {
            ...
            <div class="error notice">
                <p class="description"><?php _e("WOOBE cannot create the database table! Make sure that your mysql user has the CREATE privilege! Do it manually using your host panel phpmyadmin!", 'woocommerce-bulk-editor') ?></p>
  ...
  KEY `user_id` (`user_id`)
    ...
    public function write($field_key, $product_id, $post_parent = 0) {
        ...
        try {
            $wpdb->insert($this->table, array(
                ...
                'bulk_key' => isset($_REQUEST['woobe_bulk_key']) ? $_REQUEST['woobe_bulk_key'] : NULL,
                'user_id' => get_current_user_id()
            ));
        } catch (Exception $e) {
            print_r($e);
    ...
    public function woobe_history_get_bulk_count() {
        global $wpdb;
        $user_id = get_current_user_id();
        die($wpdb->get_var("SELECT COUNT(*) FROM {$this->table} WHERE bulk_key = '{$_REQUEST['bulk_key']}' AND user_id={$user_id}"));
    }
