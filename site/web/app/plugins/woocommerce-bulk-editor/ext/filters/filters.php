...
final class WOOBE_FILTERS extends WOOBE_EXT {
    ...
    public function __construct() {
        ...
        //tabs
        $this->add_tab($this->slug, 'top_panel', __('Filters', 'woocommerce-bulk-editor'));
    ...
    public function woobe_ext_scripts() {
        ...
        <script>
            ...
            lang.<?php echo $this->slug ?>.filtering = "<?php echo __('Filtering', 'woocommerce-bulk-editor') ?> ...";
            lang.<?php echo $this->slug ?>.filtered = "<?php echo __('Filtered! Table redrawing ...', 'woocommerce-bulk-editor') ?>";
        </script>
        <?php
    }
    ...
    public function posts_product_url_where($where = '') {
        ...
        $woobe_product_url_request = trim(urldecode($_REQUEST['woobe_product_url_search']));

        //***
        $product_url_logic = $_REQUEST['woobe_product_url_search_behavior'];
...
//echo $where;
    ...
    public function stock_quantity_where($where = '') {
        global $wpdb;
        $woobe_stock_quantity_from = intval((!empty($_REQUEST['stock_quantity_from'])) ? $_REQUEST['stock_quantity_from'] : 0);
        $woobe_stock_quantity_to = intval((!empty($_REQUEST['stock_quantity_to'])) ? $_REQUEST['stock_quantity_to'] : PHP_INT_MAX);
    ...
    public function sale_price_where($where = '') {
        global $wpdb;
        $woobe_sale_from = floatval((!empty($_REQUEST['sale_price_from'])) ? $_REQUEST['sale_price_from'] : 0);
        $woobe_sale_to = floatval((!empty($_REQUEST['sale_price_to'])) ? $_REQUEST['sale_price_to'] : PHP_INT_MAX);
    ...
    public function regular_price_where($where = '') {
        global $wpdb;
        $woobe_sale_from = floatval((!empty($_REQUEST['regular_price_from'])) ? $_REQUEST['regular_price_from'] : 0);
        $woobe_sale_to = floatval((!empty($_REQUEST['regular_price_to'])) ? $_REQUEST['regular_price_to'] : PHP_INT_MAX);
    ...
    public function woobe_apply_query_filter_data($args) {
        ...
        if (isset($woobe_filter['regular_price']) AND (!empty($woobe_filter['regular_price']['from']) OR!empty($woobe_filter['regular_price']['to']))) {
        ...
        if (isset($woobe_filter['sale_price']) AND (!empty($woobe_filter['sale_price']['from']) OR!empty($woobe_filter['sale_price']['to']))) {
        ...
        if (isset($woobe_filter['stock_quantity']) AND (!empty($woobe_filter['stock_quantity']['from']) OR!empty($woobe_filter['stock_quantity']['to']))) {
        ...
        &nbsp;|&nbsp;<span>
            ...
            <div  id="woobe_filter_form_tools_panel">
                <div class='tools_panel_filter-unit-wrap'>
                    <div class="col-lg-6">
                        <input type="text" placeholder="<?php _e('quick search by ...', 'woocommerce-bulk-editor') ?>" name="woobe_filter_form_tools_value" value="" />
                    </div>
                    <div class="col-lg-2">
