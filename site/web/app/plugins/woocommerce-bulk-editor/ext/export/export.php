...
final class WOOBE_EXPORT extends WOOBE_EXT {
    ...
    public function __construct() {
        ...
        //tabs
        $this->add_tab($this->slug, 'top_panel', __('Export', 'woocommerce-bulk-editor'));
    ...
    public function woobe_ext_scripts() {
        ...
        <script>
            ...
            lang.<?php echo $this->slug ?>.want_to_export = '<?php _e('Should the export be started?', 'woocommerce-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.exporting = '<?php _e('Exporting', 'woocommerce-bulk-editor') ?> ...';
            lang.<?php echo $this->slug ?>.exported = '<?php _e('Exported', 'woocommerce-bulk-editor') ?> ...';
            lang.<?php echo $this->slug ?>.export_is_going = "<?php echo __('ATTENTION: Export operation is going!', 'woocommerce-bulk-editor') ?>";
    ...
    public function woobe_export_products_count() {
        ...
        switch ($_REQUEST['format']) {
            ...
            case 'xml':
                ...
                if (!empty($active_fields)) {
                    $file_path = $this->get_ext_path() . '__exported_files/woobe_exported.xml';
...
//print_r($products_ids);
                    ...
                    break;
                case 'xml':
                        $file = $this->get_ext_path() . '__exported_files/woobe_exported.xml';   
                        $dom = new DOMDocument("1.0", "utf-8");
                        $dom->load( $file );
                        $rss = $dom->firstChild; 
                        $root=$rss->firstChild; 
    ...
    private function filter_fields_vals($value, $field_key, $field, $product_id) {
        $words = array(
                /*
                  'draft' => __('draft', 'woocommerce-bulk-editor'),
                  'publish' => __('publish', 'woocommerce-bulk-editor'),
                 */
    ...
    public function woobe_bulk_get_att_terms_export() {
        ...
        if (!empty($_REQUEST['attributes'])) {
            foreach ($_REQUEST['attributes'] as $pa) {
                ...
                if (!empty($terms)) {
                    $options = array();
                    $options[''] = __('not selected', 'woocommerce-bulk-editor');
                    foreach ($terms as $t) {
